...
var DEFAULT_TEMP_EXCLUDE_REASONS = ["Krankheit", "ungewöhnlich späte Messzeit"];
...
function loadSettings() {
  try {
    ...
    var manualCoverline = saved.manualCoverline && typeof saved.manualCoverline === 'object' ? Object.assign({}, saved.manualCoverline) : {};
    var manualCoverlineEnabled = saved.manualCoverlineEnabled && typeof saved.manualCoverlineEnabled === 'object' ? Object.assign({}, saved.manualCoverlineEnabled) : {};
    return { tempExcludeReasons: reasons, excludedCycles: excludedCycles, manualCoverline: manualCoverline, manualCoverlineEnabled: manualCoverlineEnabled };
...
function csvToEntries(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    entries[date] = {
      ...
      tempExclude: (getVal(vals, 'tempExclude') || '').toLowerCase() === "true",
      tempExcludeReason: parseReasons(getVal(vals, 'tempExcludeReason')),
      bleeding: parseInt(getVal(vals, 'bleeding')) || 0,
      spotting: (getVal(vals, 'spotting') || '').toLowerCase() === "true",
      mucus: parseInt(getVal(vals, 'mucus')) || 0,
      mucusExclude: (getVal(vals, 'mucusExclude') || '').toLowerCase() === "true",
